  ...
  if (req.method === 'GET') {
    try {
      ...
      const qrCodes = await prisma.qRCode.findMany({
        ...
        select: {
          id: true, qrData: true, lastLink: true, uniqueCode: true, cornerShape: true, eyeShape: true, qrShape: true,
            foregroundColor: true, backgroundColor: true, dotColor: true, cornerColor: true, eyeColor: true, frameStyle: true, 
            frameText: true, frameTextSize: true, frameColor: true, frameTextColor: true, updateCount: true,
            createdAt: true, updatedAt: true, userId: true, qrCodeImagePath: true, qrStatus: true,
    ...
    if (req.headers['content-type'] && req.headers['content-type'].includes('application/json')) {
      try {
        ...
        const {
          qrType, qrData,metadata, cornerShape, eyeShape, qrShape, foregroundColor, backgroundColor, dotColor, 
          cornerColor, eyeColor, frameStyle, frameText, frameTextSize, frameColor, frameTextColor, userId,
        } = body;
        ...
        const qrCode = await prisma.qRCode.create({
          data: {
            ...
            cornerShape, eyeShape, qrShape, foregroundColor, backgroundColor, dotColor, cornerColor, eyeColor, frameStyle, 
            frameText, frameTextSize, frameColor, frameTextColor, updateCount: 0, 
       ...
       // ✅ Also save into DigitalBusinessCard if type = businesscard
          if (qrType === "businesscard") {
            console.log("businesscard"+qrCode.uniqueCode+qrData.qrCodeImagePath)
            await prisma.digitalBusinessCard.create({
              data: {
                userId: parseInt(userId), // logged-in user ID, always number
                ...
                template: "classic",
                qrCodePath: "", // will be updated once image is uploaded
              },
            });
          }
        ...
        return res.status(200).json({ ...qrCode, serverLink });
      } catch (error) {
