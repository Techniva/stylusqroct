...
import QrStatusToggle from "@/app/components/layout/tabs/QrStatusToggle";
import MyQRReportPDF from "@/app/components/layout/tabs/QRReportPDF";
import { formatQRDataToURL } from "@/app/lib/qrDataUtils";
...
const MyQRCodes: React.FC<MyQRCodesProps> = ({ url, onStatsChange }) => {
  ...
  const handleDownload = async (qr: QRCode) => {
    try {
      const res = await fetch(`/api/qr/download/${qr.id}`, {credentials: 'include',});
      if (!res.ok) return;
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/qr/${qrToDelete.id}`, {credentials: 'include', method: 'DELETE' });
      if (response.ok) {
  ...
  console.log('User in MyQRCodes:', user);
  if (!user) {
    return (
  ...
  const loadQRCodes = async (currentUser: { id: number; fullName: string; email: string }) => {
    setIsLoading(true);
    setMessage("");
  
    try {
      if (!currentUser || !currentUser.id || isNaN(currentUser.id)) {
        console.error('Invalid or missing user:', currentUser);
      ...
      const qrRes = await fetch(`/api/qr?userId=${currentUser.id}`, { credentials: "include" });
      if (qrRes.ok) {
        const qrData = await qrRes.json();
        setQrCodes(qrData);
        setMessage(qrData.length === 0 ? "No QR codes found for your account." : "");
  
        if (qrData.length > 0) {
          const scanRes = await fetch(`/api/qr/scan-counts?userId=${currentUser.id}`, { credentials: "include" });
          if (scanRes.ok) {
        ...
        const errorData = await qrRes.json().catch(() => ({ error: 'Unknown error' }));
        setMessage(`Failed to load QR codes: ${errorData.error}`);
        setQrCodes([]);
        setQrScanCounts({});
      }
    } catch (error) {
  ...
  const fetchScanLocations = async (qrId: number) => {
    ...
    try {
      const response = await fetch(`/api/qr/scan-locations?qrCodeId=${qrId}`, { credentials: 'include' });
      if (response.ok) {
  ...
  useEffect(() => {
    if (openAnalyticsId) {
      if (!qrStats[openAnalyticsId]) {
        ...
        fetch(`/api/qr/stats?qr_code_id=${openAnalyticsId}`, { credentials: 'include' })
  ...
  useEffect(() => {
    if (user) {
      fetch(`/api/qr/scan-counts?userId=${user.id}`, { credentials: 'include' })
        .then(res => res.ok ? res.json() : Promise.reject())
    ...
    if (qrCodes.length > 0) {
      qrCodes.forEach(qr => {
        if (qr && qr.id && !(qr.id in qrScanCounts)) {
          fetch(`/api/qr/scan-counts?qrCodeId=${qr.id}`, { credentials: 'include' })
            .then(res => res.ok ? res.json() : Promise.reject())
            .then(data => {
      ...
      {qrCodes.length === 0 && !isLoading ? (
        ...
        <div className="space-y-5">
          {paginatedQRCodes.map((qr) => (
            ...
            >
              <div className="absolute top-0 left-4 -translate-y-1/2 bg-white px-1 text-xs text-gray-700">
            ...
            >
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                ...
                <div className="p-6 flex flex-col gap-4">
                  ...
                  {/* Edit Button */}
                  <button
                    className="w-full bg-green-600 text-white py-3 px-4 rounded-full hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                    disabled={isDBCLoading}
                    onClick={() => {
                      setIsDBCLoading(true); // Start loading state
                  ...
                  {/* Cancel Button */}
                  <button
                    className="w-full bg-gray-300 text-gray-700 py-3 px-4 rounded-full hover:bg-gray-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    onClick={handleCancelEdit}
                    disabled={isDBCLoading}
                  >
                    Cancel
